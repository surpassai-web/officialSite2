  ...
  var smartH$ = (window.smartH$ = new (function () {
    ...
    function xb() {
      if (!C) {
        ...
        var a;
        if (v.pointerEnabled || (a = v.msPointerEnabled))
    ...
    function m(l) {
      if (!j) {
        j = -1;
        if (
          kb == "Microsoft Internet Explorer" &&
          !!window.attachEvent &&
  ...
  smartAm.prototype = {
    ...
    play: function () {
      ...
      if (
        !device.$Touchable &&
        smartH$.$IsBrowserIE() &&
        smartH$.$IsBrowserIe10Earlier() < 10
      ) {
        this.playIE();
      } else {
        this.playNoraml();
    ...
    playNoraml: function () {
      ...
      this.end("AnimationEnd", function () {
        ...
        if ($me.$element.hasClass($me.queue[0])) {
          if (
            !$me.options.keep &&
            !($me.queue.length == 0 || $me.options.infinite === "infinite")
          ) {
            $me.cleanse();
          }
          $me.queue.shift();
    ...
    animate: function () {
      ...
      if (
        this.abovethetop(this.$element, this.options) ||
        this.leftofbegin(this.$element, this.options)
      ) {
        /* Nothing. */
      } else if (
        typeof isfinished === "undefined" &&
        !this.belowthefold(this.$element, this.options) &&
        !this.rightoffold(this.$element, this.options) &&
        typeof this.$element.attr("sm-finished") === "undefined"
      ) {
        this.play();
      }
    },
...
+(function ($) {
  ...
  var SmRecompute = function (element, options) {
    ...
    this.$relationEles = this.$element
      .siblings(":not(.smv-line)")
      .filter(function () {
        var $this = $(this);
        var w = $this.width();
        var p = $this.position();
        ...
        return (
          t > startTop &&
          !$this.hasClass("smartFixed") &&
          ((l >= startLeft && l <= endLeft) ||
            (lw >= startLeft && lw <= endLeft) ||
            (startLeft >= l && startLeft <= lw) ||
            (endLeft >= l && endLeft <= lw))
        );
      });
  };
  SmRecompute.prototype = {
    ...
    recomputeHTo: function (newH, hisH) {
      ...
      var result = this.innerRecompute(
        this.$element,
        this.$relationEles,
        expandH,
        true
    ...
    rejustClostParentH: function (pvid, ele, expandData) {
      ...
      if (
        closetParentId !== "smv_" + pvid &&
        $closetParent.hasClass("smAreaC") &&
        !$closetParent.hasClass("expandFlag")
      ) {
        $closetParent
          .addClass("expandFlag")
          .height($closetParent.height() + expandData.expandH);
      } else {
    ...
    return elements.each(function () {
      ...
      if (!data) {
        if (typeof smOption === "object") {
          $this.data(
            "smart.remcopute",
            (data = new SmRecompute(this, smOption))
          );
          data.recomputeH();
        } else {
